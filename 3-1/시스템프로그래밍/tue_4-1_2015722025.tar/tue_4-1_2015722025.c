...
//------------------------------------------------------------------//
//Title : System Programing Assignment #4-1 (Web_server)            //
//Description : The 4-1 task is based on 3-3 and, when a request is //
//              received from the original client, the process is   //
//              created through the fork at the moment, but this    //
//              task is aimed at reducing the time to fork when     //
//              the request is received by using pre-fork           //
//              technology. Other than that, the goal is to         //
//              understand and learn more about the use of signal.  //
//////////////////////////////////////////////////////////////////////
#define _GNU_SOURCE
#include<stdio.h>
#include<stdlib.h>
...
void UpdateInfo(struct sockaddr_in client_addr,int PID)
{
	...
	{
		int count=0;
		p_info* tmp=Info_Head;
		
		newnode->Num=Con_count+1;
		strcpy(newnode->IP,inet_ntoa(client_addr.sin_addr)); //declare information of node
		newnode->Portnum=client_addr.sin_port;
		newnode->Pid=PID;
		strcpy(newnode->Atime,time_buf);
		...
		tmp->next=newnode; //For link node
