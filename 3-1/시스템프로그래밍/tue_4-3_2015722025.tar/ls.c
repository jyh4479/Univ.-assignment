...
//------------------------------------------------------------------//
//Title : System Programing Assignment #4-3 (Log file and Semaphore)//
//Description : The 4-3 task is to prepare log file through         //
//              semaphore based on the tasks that have been carried //
//              out so far. Semaphore is a technology that controls //
//              the access of a process, preventing simultaneous    //
//              access to the log using semaphore for use with      //
//              processes sharing and using the function of writing //
//              the log.                                            //
//////////////////////////////////////////////////////////////////////
#define _GNU_SOURCE
#include<stdio.h>
...
FILE *LOG_FILE;
sem_t *mysem;
char log_buffer[500]={0, };
char sub_buffer[500]={0, };
char init_buffer[500]={0, };

pthread_t log_id;
void practice_log(void * arg);
//////////////////////////////////////////////////////////////////////
...
void signalHandler_parent(int sig)
{
	...
	if(sig == SIGALRM) //alram signal
	{
		...
		sprintf(sub_buffer,"========= Connection History =========\n");
		strcat(log_buffer,sub_buffer);
		sprintf(sub_buffer,"NO.\tIP\t\tPID\tPORT\tTIME\n");
		strcat(log_buffer,sub_buffer);
		
		pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
		pthread_join(log_id,NULL);
		strcpy(log_buffer,init_buffer);
	...
	{
		pthread_create(&log_id,NULL,(void*)practice_log,"^C\n");
		pthread_join(log_id,NULL);
		strcpy(log_buffer,init_buffer);
		...
		for(int i=0;i<MaxChilds;i++) //for printing condition of exit
		{
			...
			if(IDLE_P[i]!=0)
			{
				if(waitpid(IDLE_P[i],&status,0)!=-1) //wait child process
				{
					...
					sprintf(log_buffer,"[%s] %d process is terminated.\n",time_buf,IDLE_P[i]);
					pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
					pthread_join(log_id,NULL);
					strcpy(log_buffer,init_buffer);
		...
		sprintf(log_buffer,"[%s] Server is terminated.\n",time_buf);
		pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
		pthread_join(log_id,NULL);
		strcpy(log_buffer,init_buffer);
...
void *doit4(void *vptr) //Just create process
{
	...
	sprintf(log_buffer,"[%s] IdleProcessCount : %d\n",time_buf,shm_info->idle);
	pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
	pthread_join(log_id,NULL);
	strcpy(log_buffer,init_buffer);
...
// log thread                                                       //
// ================================================================ //
// Input: void * -> get string                                      //
//                                                                  //
// Output:                                                          //
//                                                                  //
// Purpose: For writing log history                                 //
//////////////////////////////////////////////////////////////////////
void practice_log(void *arg)
{
	mysem = sem_open("40229", O_RDWR); //open semaphore
	sem_wait(mysem); //lock

/////////////////critical section////////////////////////
	LOG_FILE = fopen("server_log.txt", "a"); //open file
	fprintf(LOG_FILE, "%s", (char *)arg); //write string
	fclose(LOG_FILE); //close file
/////////////////critical section////////////////////////
	
	sem_post(mysem); //unlcok
	return;
}
...
int child_main(int socket_fd)
{
	...
	while (1) //start server
	{
		...
		//}
			...
			struct timeval start_Time, end_Time, run_Time;
			double diffTime;
			...
			sprintf(sub_buffer,"============= New client =============\n");
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"TIME : [%s]\n",time_buf);
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"URL : %s\n",url);
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"IP : %s\n",inet_ntoa(client_addr.sin_addr));
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"Port : %d\n",client_addr.sin_port);
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"PID : %d\n",getpid());
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"======================================\n");
			strcat(log_buffer,sub_buffer);
			...
			printf("Port : %d\n",client_addr.sin_port);
			printf("PID : %d\n",getpid());
			printf("CONNECTING TIME: %ld(us)\n",run_Time.tv_usec);
			printf("======================================\n");

			sprintf(sub_buffer,"========= Disconeected Client ========\n");
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"TIME : [%s]\n",time_buf);
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"URL : %s\n",url);
			...
			sprintf(sub_buffer,"PID : %d\n",getpid());
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"CONNECTING TIME: %ld(us)\n",run_Time.tv_usec);
			strcat(log_buffer,sub_buffer);
			sprintf(sub_buffer,"======================================\n");
			strcat(log_buffer,sub_buffer);
...
{
	if((LOG_FILE=fopen("server_log.txt","w"))<0)
	{
		printf("Fail open file\n");
		return 0;
	}
	mysem = sem_open("40229", O_CREAT | O_EXCL, 0700, 1); //setting semaphore
	sem_close(mysem); //close
	...
	sprintf(log_buffer,"[%s] Server is started.\n",time_buf);
	pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
	pthread_join(log_id,NULL);
	strcpy(log_buffer,init_buffer);
	...
	if((conf_File=fopen("httpd.conf","r"))<0)
	{
		printf("Fail open file\n");
		return 0;
	}
...
///////////////////////////Make child process//////////////////////////////////////
	...
	for(int i=0;i<StartProcess;i++)
	{
		if((PID=fork())>0) //parent process
		{
			...
			sprintf(log_buffer,"[%s] %d process is forked.\n",time_buf,PID);
			pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
			pthread_join(log_id,NULL);
			strcpy(log_buffer,init_buffer);
...
{	
	...
	{ //End list of print 10
		...
		sprintf(log_buffer,"%d\t%s\t%d\t%d\t%s",tmp->Num,tmp->IP,tmp->Pid,tmp->Portnum,tmp->Atime);
		pthread_create(&log_id,NULL,(void*)practice_log,log_buffer);
		pthread_join(log_id,NULL);
		strcpy(log_buffer,init_buffer);
	}
